...
구분|설명
:---:|:---:
데이터 프로퍼티`data property` | 키와 값으로 구성된 일반적인 프로퍼티 <br> 지금까지 살펴본 프로퍼티
접근자 프로퍼디`accessor property` | 자체적으로는 값을 갖지 않고 다른 데이터 프로퍼티의 값을 읽거나 저장할 때 호출되는 접근자 함수`accessor function`로 구성된 프로퍼티(get, set)
